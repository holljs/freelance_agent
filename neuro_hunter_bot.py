...
def compare_models(models, peer_id):
    ...
    for model in models:
        ...
        system_prompt = f"""Ты — AI-аналитик. Перед тобой модель из китайского хаба SiliconFlow.
Твоя задача — просто человеческим языком на русском объяснить Наташе, что это за модель и для чего она нужна (это текст, картинки или видео).
Сравни её с нашим стеком: {STACK_JSON}. 
Напиши 2-3 предложения: подходит ли она как аналог для наших 'Цехов' (особенно если это Flux, DeepSeek или Wan), работает ли без VPN и почему в Китае её запускать выгоднее.
Никаких 'ИГНОР' писать не нужно! Расскажи про модель в любом случае."""
